...
# Reset ball with default speed after scoring
def reset_ball():
    ball.goto(0, 0)
    ball.dx = 0.8 if ball.dx > 0 else -0.8  # Ensure proper direction
    ball.dy = -0.8  # Keeps movement balanced

# Border checking left and right.
if ball.xcor() > 390:  # Ball goes past the right border, Player A scores
    score_a += 1
    reset_ball()
    pen.clear()
    pen.write(f"Player A: {score_a}  Player B: {score_b}", align="center", font=("Courier", 24, "normal"))

if ball.xcor() < -390:  # Ball goes past the left border, Player B scores
    score_b += 1
    reset_ball()
    pen.clear()
    pen.write(f"Player A: {score_a}  Player B: {score_b}", align="center", font=("Courier", 24, "normal"))
